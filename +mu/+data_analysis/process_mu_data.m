function analysed_mu_data = process_mu_data()
    % PROCESS MU DATA
    ...
    mu_data = processMUData(CONFIG.MU_FOLDER, timing_data, force_data, max_forces);
    
    fprintf('\nStep 4: Cleaning and labeling data...\n');
    analysed_mu_data = cleanAndLabelData(mu_data, CONFIG.GROUPS);

    fprintf('\nProcessing complete. Data is output as analysed_mu_data.\n');
end
...
function mu_data = processMUData(mu_folder, timing_data, force_data, max_forces)
    % Processes all motor unit files with corresponding force and timing data
    mu_data = process_mu_files_with_thresholds(mu_folder, timing_data, force_data, max_forces);
end
...
function analysed_mu_data = cleanAndLabelData(data, groups)
    ...
    % - At least 3 interpulse intervals
    ...
    analysed_mu_data = sorted_data(:, {'participant', 'testing_group', 'force_level', 'trial', ...
        'muscle', 'accuracy', 'avg_firing_rate', 'cov_ipi', 'num_ipi', ...
...
function MU_data = calculate_firing_rate(input_decomp_data, start_time, end_time, force_data, max_force)
    ...
    % Initialise structure to hold all metrics
...
function metrics = initialiseMetrics(num_units)
    ...
    fields = {'avg_firing_rate', 'cov_ipi', 'num_ipi', 'accuracy', ...
              'firing_threshold', 'initial_firing_time'};
...
function [rate, cov, num] = calculateFiringMetrics(firing_times, start_time, end_time)
    ...
    % Calculate metrics:
    avg_IPI = mean(IPIs);
    rate = 1 / avg_IPI;                          % Convert IPI to frequency
...
function table_out = createOutputTable(metrics)
    ...
    table_out = table(metrics.accuracy, metrics.avg_firing_rate, ...
                     metrics.cov_ipi, metrics.num_ipi, ...
                     metrics.firing_threshold, metrics.initial_firing_time, ...
                     'VariableNames', {'accuracy', 'avg_firing_rate', 'cov_ipi', ...
                                     'num_ipi', 'firing_threshold', 'initial_firing_time'});
end

function all_MU_data = process_mu_files_with_thresholds(mu_folder, timing_data, force_data, max_forces)
    ...
    for i = 1:length(files)
        try
            ...
            result = processSingleFile(files(i), timing_data, force_data, max_forces, mu_folder);
...
function result = processSingleFile(file, timing_data, force_data, max_forces, mu_folder)
    ...
    MU_data = calculate_thresholds(decomp_data, force_profile, max_force, start_time, end_time);
...
function MU_data = calculate_thresholds(input_decomp_data, force_data, maxforce, start_time, end_time)
    % Calculate basic metrics first using the existing function
    MU_data = calculate_firing_rate(input_decomp_data, start_time, end_time, force_data, maxforce);
