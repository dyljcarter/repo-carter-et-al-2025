function [coherence, inputdata, cov, pooled_coherence, comparison_of_coherence] = calculate_coherence()
...
addpath(fullfile(pwd, '+coherence','+data_analysis', 'neurospec20', 'neurospec20'));
...
[pooled_coherence, comparison_of_coherence] = calculate_pooled_coherence(pooled_participant_coherence, CONFIG);
end
...
CONFIG.SAMPLING_RATE = 2000;                    % Data sampling rate
CONFIG.DATA_PATH = fullfile(pwd, '+coherence', '+data_analysis', '+emg_force_data', filesep);
end
...
[force_cov_mean, force_cov_target] = calculate_force_metrics(force_data, max_force);
...
cov_out = struct('forceCOVmean', force_cov_mean, 'forceCOVtarget', force_cov_target);
end
...
for i = 1:length(config.FORCE_LEVELS)
    for j = 1:length(config.REPS)
        ...
        % Load and filter EMG data
        emg = readtable(filename);
        emg = table2array(emg);
        [B, A] = butter(2, 20 / (config.FS / 2), 'high'); % high-pass 2nd order butterworth filter at 20Hz
        emg = [emg(:,1), filtfilt(B, A, emg(:,2:9))];
        emg_data(:,:,j,i) = emg;
    end
end
end
...
function [force_cov_mean, force_cov_target] = calculate_force_metrics(force_data, max_force)
% Calculate force coefficient of variation metrics
%
% Calculates two types of COV:
%   1. Standard COV (std/mean)
%   2. Target-based COV (std/target_force)
%
% Parameters:
...
%
% Returns:
%   force_cov_mean - Mean COV across repetitions
%   force_cov_target - Target-based COV

steady_force = force_data(14000:26000, 1:2, :, :);

% Calculate mean COV across repetitions
force_cov_mean = zeros(3, 4);
for i = 1:4
    for j = 1:3
        force_cov_mean(j,i) = getcov(steady_force(:,2,j,i));
    end
end
force_cov_mean = mean(force_cov_mean, 1);
...
for i = 1:4
    for j = 1:3
        ...
        [f(:,:,j,i), t(:,:,j,i), cl(:,:,j,i), sc(:,:,j,i)] = ...
            sp2a2_m1(0, steady_emg(:,2), steady_emg(:,6), config.FS, 10, 'n', 'r2');
    end
end
...
function [pooled_coherence, comparison] = calculate_pooled_coherence(pooled_participant_coherence, config)
% Calculate pooled coherence across participants and compare between groups
%
% Two main steps:
%   1. Pool coherence across participants within each group
%   2. Compare coherence between groups using the neurospec20 toolbox's 
%      sp2_compcoh
%
% Parameters:
%   pooled_participant_coherence - Coherence pooled within participants
%   config - Analysis configuration parameters
%
% Returns:
...
for groupIdx = 1:length(group_names)
    group_name = group_names{groupIdx};
    participants = fieldnames(pooled_participant_coherence.(group_name));
    ...
    for i = 1:length(force_level_labels)
        ...
        if groupIdx == 1
            fprintf('\nPooling Coherence across Paricipants for Dexterity-Trained at %sMVC:\n',config.FORCE_LEVELS{i})
        else
            fprintf('\nPooling Coherence across Participants for Strength-Trained at %sMVC:\n',config.FORCE_LEVELS{i})
        end

        % Pool across participants
        for participant_loop = 1:length(participants)
            current_participant = participants{participant_loop};

            if participant_loop == 1
                % initialise pooling with first participant
                [out_f, out_v] = pool_scf(...
                    pooled_participant_coherence.(group_name).(current_participant).(current_force_level).sc, ...
                    pooled_participant_coherence.(group_name).(current_participant).(current_force_level).cl);
            else
                % Add subsequent participants to pool
                [out_f, out_v] = pool_scf(...
                    pooled_participant_coherence.(group_name).(current_participant).(current_force_level).sc, ...
                    pooled_participant_coherence.(group_name).(current_participant).(current_force_level).cl, ...
                    in_f, in_v);
